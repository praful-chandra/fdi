...
const {
  signatureRequest1,
  signatureResponse1,
} = require("../cashFree.helpers/signatureCreation");
const {
  transactionStatusEnum,
  paymentOptions,
} = require("../cashFree.helpers/enums");
...
router.post("/cashfree", async (req, res) => {
  ...
  res.json({
    signatureForm,
    path: paths.prod.cashfreePayUrl,
  });
});
        ...
        if (signature !== fields.signature) {
          ...
          switch (fields.txStatus) {
            ...
            case txnTypes.success: {
              ...
              order.cart.map((oc) => {
                itemLists.push({
                  product: oc.product.productId,
                  quantity: oc.quantity,
                });
              });

              for (let i = 0; i < itemLists.length; i++) {
                await ProductVarianceColor.findByIdAndUpdate(
                  itemLists[i].product,
                  {
                    $inc: {
                      quantity: - itemLists[i].quantity,
                    },
                  }
                );
              }
          ...
          return res.redirect(
            `http://localhost:3000/paymentStatus?order=${fields.orderId}`
          );
        }
      } catch (err) {
        return res.status(500).json({
