...
function CreateFDIComponent({ reset }) {
  ...
  useEffect(async () => {
    ...
    try {
      ...
      if (categoriesList.error || subCategoryList.error) throw new Error();
  ...
  const handleSearch = async (txt) => {
    ...
    if (!txt) {
      ...
      const getProducts = await listProduct(5, 0, {
        name: txt,
        category,
        subCategory,
      });
      ...
      <div className={styles.selectColor}>
        ...
        <div className={styles.selectColorBody}>
          {selectedProduct.images[0] && (
            <img
              src={`${process.env.REACT_APP_API_ROOT_URI}${selectedProduct.images[0].full}`}
              className={styles.selectColorImage}
              alt=""
            />
          )}{" "}
          <div className={styles.selectColorData}>
            {selectedProduct.options.map((optns) => {
              return (
                <div
                  key={`select col option ${optns._id}`}
                  className={styles.selectColorOption}
                >
                  <p>{optns.title}</p>
                  <table className="table">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">FDI RECOMMENDED</th>
                      </tr>
                    </thead>
                    <tbody>
                      {optns.color.map((col) => (
                        <ColorListComponent
                          key={`select col col ${col._id}`}
                          col={col}
                        />
                      ))}
                    </tbody>
            ...
            <Form.Item label="Category">
              ...
              >
                {categories.list.map((cat) => (
                  <Option key={`dropcat ${cat._id}`} value={cat._id}>
                    {" "}
                    {cat.name}
                  </Option>
                ))}
            ...
            <Form.Item label="Sub Category">
              ...
              >
                {subCategories.list.map((cat) => (
                  <Option key={`dropSubcat ${cat._id}`} value={cat._id}>
                    {" "}
                    {cat.name}
                  </Option>
                ))}
...
export default CreateFDIComponent;
